...
namespace
{
                             ...
                             const char * const name)
    {
        ...
        if (not result.isValid()) {
            ...
            const QByteArray &cachedName(cache.value(ba));
            if (not cachedName.isEmpty()) {
                return obj->property(cachedName);
            }

            bool turnUp = false;
            // Blindly assume that property default names start with "maliit-".
            // Tested by Ut_MInputContext::testPropertyNameNormalization().
            for (int idx = 7; idx < ba.length(); ++idx) {
                const QChar curr = ba.at(idx);
                if (curr == '-') {
                    turnUp = true;
                } else {
