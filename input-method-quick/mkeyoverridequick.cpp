...
void MKeyOverrideQuick::applyOverride(const QSharedPointer<MKeyOverride>& keyOverride, const MKeyOverride::KeyOverrideAttributes changedAttributes)
{
    Q_D(MKeyOverrideQuick);
    ...
    if (keyOverride)
    {
        // only one of label and icon can be shown at the same time on one key.
