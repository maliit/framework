...
void MImOnScreenPlugins::updateActiveSubview()
{
    ...
    if (!isSubViewEnabled(subView)) {
        qDebug() << __PRETTY_FUNCTION__ << "enabling activated subview" << subView.plugin + ":" + subView.id << "automatically";
        ...
        // Check invariant: active subview should only be present once in enabled list
