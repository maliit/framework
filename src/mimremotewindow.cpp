...
static const Atom get_wm_state_atom()
{
...
bool MImRemoteWindow::wasIconified(XEvent *ev) const
{
    ...
    if (ev->xproperty.atom == get_wm_state_atom()) {
        return isIconified();
    }
...
# define IS_ICONIFIED(_X) _X == IconicState
    ...
    int queryResult = XGetWindowProperty(QX11Info::display(), wid, get_wm_state_atom(), 0, 2,
